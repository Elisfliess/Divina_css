...
$subcategorias = []; // Inicializa como array vazio
...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM subcategoria"); //vai buscar todas as colunas da tabela ADMINISTRADOR
    $stmt->execute();  //***vide explicações sobre a dinâmica desses comandos no final do arquivo
    $subcategorias = $stmt->fetchAll(PDO::FETCH_ASSOC); //fetch = recuperar, buscar
...
<head>
    <meta charset="UTF-8">
    <title>Listar Subategorias</title>
...
<table>
    ...
    <a href="exportar_excel.php?tipo=subcategoria" target="_blank">Exportar Subcategorias</a><br>
      <p></p>
    <a href="cadastrar_subcategorias.php">Cadastre nova categoria</a>
      <p></p>
